...
if __name__ == "__main__":
    ...
    for eachsite in sitelist:
        ...
        PEMODfaparstd=MODdf["FAPARstd"].values.tolist()
        ...
        PEMYDfaparstd = MYDdf["FAPARstd"].values.tolist()
        ...
        PEVNPfaparstd = VNPdf["FAPARstd"].values.tolist()
        ...
        PEPROBAV300faparstd = PROBAV300df["FAPARstd"].values.tolist()
        ...
        PEPROBAV1000faparstd = PROBAV1000df["FAPARstd"].values.tolist()
        ...
        if subi == 1:
            ax.legend(loc='lower left', bbox_to_anchor=(0.42, 0.65), ncol=5, frameon=False, labelspacing=0.1,handlelength=1,handletextpad=0.2,columnspacing=1,prop=legendfont)

        xtic = []
        for i in range(2013, 2018):
            xtic.append(float(str(i) + '001'))
            plt.xticks(xtic, fontsize=fontsize+12)
        ax.axis([2012890, 2018610, -0.08, 0.3])
        ...
        if subi==3:
            ax.set_ylabel("Quantitative quality indicators", fontdict=font,y=1.1)
        # if subi==1:
    ...
    plt.savefig('Fig6.Temporal_QQI2.png')  # 保存图片
