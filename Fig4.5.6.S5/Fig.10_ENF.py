    ...
    for eachSite in Site:
        ...
        for eachProduct in Products:
            ...
            if eachProduct in ["MOD","MYD","VNP"]:
                TP4df = pd.read_csv(r".\Data\CA-TP4_HLS_MODIS_FAPAR_RMSE_QC.csv")
                ncols = TP4df.shape[1]
                TP4npixels = (ncols - 3) // 10

                # Uafdf = pd.read_csv(r".\Data\US-Uaf_HLS_MODIS_FAPAR_RMSE_QC.csv")
                # ncols = Uafdf.shape[1]
                # Uafnpixels = (ncols - 3) // 10

            else:
                TP4df = pd.read_csv(r".\Data\CA-TP4_HLS_"+eachProduct+"_FAPAR_RMSE_QC.csv")
                ncols = TP4df.shape[1]
                TP4npixels = (ncols - 3) // 4

                # Uafdf = pd.read_csv(r".\Data\US-Uaf_HLS_" + eachProduct + "_FAPAR_RMSE_QC.csv")
                # ncols = Uafdf.shape[1]
                # Uafnpixels = (ncols - 3) // 4
            ...
            for eachQA in QAlist:
                ...
                if QAi==1:
                    ...
                    marker1 = 3
                    marker2 = 4
                for ipixel in range(1, TP4npixels + 1):
                    tempTP4df = TP4df[((TP4df[eachProduct + "QA" + str(ipixel)] == marker1) | (TP4df[eachProduct + "QA" + str(ipixel)] == marker2)) & (TP4df[eachProduct + "FAPAR" + str(ipixel)] >0)]
                    tempZ = tempTP4df["doy"].values.tolist()
                    tempX = tempTP4df["HLS" + str(ipixel)].values.tolist()
                    tempY = tempTP4df[eachProduct + "FAPAR" + str(ipixel)].values.tolist()
                    ALLX.extend(tempX)
                    ALLY.extend(tempY)
                    ALLZ.extend(tempZ)

                # for ipixel in range(1, Uafnpixels + 1):
                #     tempUafdf = Uafdf[(Uafdf[eachProduct + "QA" + str(ipixel)] == marker1) | (Uafdf[eachProduct + "QA" + str(ipixel)] == marker2)]
                #     tempZ = tempUafdf["doy"].values.tolist()
                #     tempX = tempUafdf["HLS" + str(ipixel)].values.tolist()
                #     tempY = tempUafdf[eachProduct + "FAPAR" + str(ipixel)].values.tolist()
                #     ALLX.extend(tempX)
                #     ALLY.extend(tempY)
                #     ALLZ.extend(tempZ)
                ...
                #     continue
                if i == 1:
                    ax.text(0.02, 1.1, "(I) "+eachSite, verticalalignment='top', horizontalalignment='left',
                ...
                if i in [9,10]:
                    ...
                    ax.set_xticklabels([])
                sc = ax.scatter(ALLX, ALLY, marker="o", s=60, linewidths=0.3, edgecolors='k', c=ALLZ, vmin=1,
                                vmax=365,
                                cmap=cm)
        ...
        plt.savefig('Fig.HLS_FAPAR_QA_ENF.png')  # 保存图片
