...
if __name__ == "__main__":
    ...
    for eachdata in ["MOD", "MYD", "VNP", "PROBAV300", "PROBAV1000"]:
        for eachsite in sitelist:
            ...
            # 逐行遍历DataFrame
            for index, row in TPDdf.iterrows():
                # 在这里处理每一行的数据
                # print(f'Index: {index}, Column1: {row["Column1"]}, Column2: {row["Column2"]}')
                tempX=[]
                tempY=[]
                for ipixel in range(1, TPDnpixels + 1):
                    Xvalue=row["HLS" + str(ipixel)]
                    if Xvalue<=0:
                        continue
                    Yvalue = row[eachdata + "FAPAR" + str(ipixel)]
                    if Yvalue <=0:
                        continue
                    tempX.append(row["HLS" + str(ipixel)])
                    tempY.append(row[eachdata + "FAPAR" + str(ipixel)])
                if len(tempY)==0:
                    continue
                ALLX.append(np.mean(tempX))
                ALLY.append(np.mean(tempY))
                ALLDOY.append(row["doy"])

                # HLS1, PROBAV1000FAPAR1
            ...
            sc = ax.scatter(ALLX, ALLY, marker="o", s=60, linewidths=0.3, edgecolors='k', c=ALLDOY, vmin=1, vmax=365,
                            cmap=cm)
            ...
            #         horizontalalignment='left',
            #         transform=ax.transAxes, fontname="Times New Roman", fontsize=fontsize-5)
            # ax.text(0.02, 0.81, "R=" + str(r21), verticalalignment='bottom', horizontalalignment='left',
            #         transform=ax.transAxes, fontname="Times New Roman", color='k', fontsize=fontsize - 5)
            # # ax.text(0.02, 0.73, "P$_\mathrm{Thr}$=" + str(round(abs(URAthr) * 100, 1)) + "%", verticalalignment='bottom',
            # #         horizontalalignment='left',
            # #         transform=ax.transAxes, fontname="Times New Roman", color='k', fontsize=fontsize - 5)
            # ax.text(0.02, 0.65, "P$_\mathrm{T}$=" + str(round(abs(URAtar) * 100, 1)) + "%", verticalalignment='bottom',
            #         horizontalalignment='left',
            #         transform=ax.transAxes, fontname="Times New Roman", color='k', fontsize=fontsize - 5)
            # ax.text(0.02, 0.57, "P$_\mathrm{O}$=" + str(round(abs(URAopt) * 100, 1)) + "%", verticalalignment='bottom',
            #         horizontalalignment='left',
            #         transform=ax.transAxes, fontname="Times New Roman", color='k', fontsize=fontsize - 5)
            # # # ax.text(0.02, 0.33, r'R$^2$=' + str(r21), verticalalignment='bottom', horizontalalignment='left',
            # # #         transform=ax.transAxes, fontname="Times New Roman", color='k', fontsize=fontsize - 5)
            #
            # if SDbias1 > 0:
            #     ax.text(0.02, 0.33, r'SD=' + str(SDbias1), verticalalignment='bottom', horizontalalignment='left',
            #             transform=ax.transAxes, fontname="Times New Roman", color='k', fontsize=fontsize - 5)
            #     # ax.text(0.02, 0.25, r'RSDB=' + str(round(abs(RSDB) * 100, 1)) + "%", verticalalignment='bottom',
            #     #         horizontalalignment='left',
            #     #         transform=ax.transAxes, fontname="Times New Roman", color='k', fontsize=fontsize - 5)
            # else:
            #     ax.text(0.02, 0.33, r'SD=$-$' + str(abs(SDbias1)), verticalalignment='bottom',
            #             horizontalalignment='left',
            #             transform=ax.transAxes, fontname="Times New Roman", color='k', fontsize=fontsize - 5)
            #     # ax.text(0.02, 0.25, r'RSDB=$-$' + str(round(abs(RSDB) * 100, 1)) + "%", verticalalignment='bottom',
            #     #         horizontalalignment='left',
            ...
            # #     0.41,0.49
            # if bias1 > 0:
            #     ax.text(0.02, 0.49, r'Bias=' + str(bias1), verticalalignment='bottom', horizontalalignment='left',
            #             transform=ax.transAxes, fontname="Times New Roman", color='k', fontsize=fontsize - 5)
            #     # ax.text(0.02, 0.41, r'RMB=' + str(round(abs(RMB) * 100, 1)) + "%", verticalalignment='bottom',
            #     #         horizontalalignment='left',
            #     #         transform=ax.transAxes, fontname="Times New Roman", color='k', fontsize=fontsize - 5)
            # else:
            #     ax.text(0.02, 0.49, r'Bias=$-$' + str(abs(bias1)), verticalalignment='bottom',
            #             horizontalalignment='left',
            #             transform=ax.transAxes, fontname="Times New Roman", color='k', fontsize=fontsize - 5)
            #     # ax.text(0.02, 0.41, r'RMB=$-$' + str(round(abs(RMB) * 100, 1)) + "%", verticalalignment='bottom',
            #     #         horizontalalignment='left',
            #     #         transform=ax.transAxes, fontname="Times New Roman", color='k', fontsize=fontsize - 5)
            #
            # ax.text(0.02, 0.17, r'RMSE=' + str(rmse1), verticalalignment='bottom', horizontalalignment='left',
            #         transform=ax.transAxes, fontname="Times New Roman", color='k', fontsize=fontsize - 5)
            #
            # # ax.text(0.02, 0.09, r'RRMSE=' + str(round(rrmse * 100, 1)) + "%", verticalalignment='bottom',
            # #         horizontalalignment='left',
            ...
            #
            # if b1 > 0:
            #     ax.text(0.02, 0.01, r'y=' + str(a1) + "x+" + str(b1), verticalalignment='bottom',
            #             horizontalalignment='left',
            #             transform=ax.transAxes, fontname="Times New Roman", color='k', fontsize=fontsize - 5)
            # else:
            #     ax.text(0.02, 0.01, r'y=' + str(a1) + "x$-$" + str(abs(b1)), verticalalignment='bottom',
            #             horizontalalignment='left',
            #             transform=ax.transAxes, fontname="Times New Roman", color='k', fontsize=fontsize - 5)
    ...
    plt.savefig('FigS2_Plot.png')  # 保存图片
