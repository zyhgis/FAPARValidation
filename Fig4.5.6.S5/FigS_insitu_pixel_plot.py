...
if __name__ == "__main__":
    ...
    for eachdata in ["MOD", "MYD", "VNP", "PROBAV300", "PROBAV1000"]:
        TPDdf = pd.read_csv(eachdata + "_Field_FAPAR.csv")
        ...
        for eachsite in sitelist:
            ...
            # ALLY = []
            # ALLDOY = []
            tempdf=TPDdf[TPDdf["site"]==eachsite]
            ALLX=tempdf["FieldFAPAR"].values.tolist()
            ALLY=tempdf["RSFAPAR"].values.tolist()
            ALLDOY=tempdf["doy"].values.tolist()
    ...
    plt.savefig('FigS3_Plot.png')  # 保存图片
    outdf["N"] = Nlist
    outdf["R"] = Rlist
    outdf["RMSE"] = RMSElist
    outdf["Bias"] = MBlist
