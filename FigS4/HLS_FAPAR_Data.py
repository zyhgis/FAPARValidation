...
if __name__ == '__main__':
    ...
    FAPARProduct=["MODIS","PROBAV1000","PROBAV300"]
    for eachFAPAR in FAPARProduct:
        ...
        for eachsite in sitelist:
            ...
            for eachfile in Tiflist:
                ...
                tempyear = int(filesplit[4][0:4])
                tempdoy=int(filesplit[4][4:7])
                tempsensor=filesplit[2]
                bandi=0

                dataset, gcs, pcs, extend, shape = get_file_info(eachfile)
                img = dataset.GetRasterBand(1).ReadAsArray()
                alllen=img.shape[0]*img.shape[0]
                npvalue=img[img>0]
            ...
            df.to_csv("./Data" + s + eachsite + "_HLS_FAPAR_"+eachFAPAR+".csv")
